...
The instrumentation of applications with the API calls to rocprofiler allows for close monitoring of the type of operations being performed. The use of this tool can give valuable insights about the relation that the running code has with the utilized frequency and voltage and the produced result.
...
Relevant work is being done on finding improvements to the current GPU DVFS system. The investigation is roughly divided on the GPU DVFS characterization, on architectural changes to DVFS, and on the creation of novel control mechanisms that are aware of the type of workload. GPU DVFS characterization works explore how does the performance and energy efficiency relate to DVFS settings using different kinds of workloads. The study on architectural changes to DVFS investigates how does the creation of more clock/voltage domains benefit the energy efficiency of the device.

\subsubsection{GPU DVFS Characterization}
